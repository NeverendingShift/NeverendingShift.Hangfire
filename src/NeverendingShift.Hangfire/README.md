...
Advanced Hangfire extensions providing thread-safe access to `PerformingContext` throughout your background jobs. Works with both .NET Framework 4.8 and .NET 5+.

## Features

- ✅ **Thread-safe** context access using `AsyncLocal<T>`
- ✅ **Works with .NET Framework 4.8 and .NET 5+**
- ✅ **No Microsoft.Extensions.Logging dependency** (works everywhere)
...
### Conditional Logging (Works Inside and Outside Jobs)
...
public class SharedService
{
    ...
    public async Task DoWorkAsync()
    {
        if (_contextAccessor.IsInJobContext())
        {
            ...
            _contextAccessor.WriteLine("Processing work item");
...
// Write to Hangfire console
void WriteLine(this IPerformingContextAccessor accessor, string message, ConsoleTextColor? color = null)
...
### 4. Progress Values Must Be 0-100
```csharp
// ✅ Good
_contextAccessor.SetProgress(75, "Almost done");

// ❌ Bad - throws ArgumentOutOfRangeException
_contextAccessor.SetProgress(150);
_contextAccessor.SetProgress(-10);
```
...
await Task.WhenAll(
    ProcessJobAsync("job-1"),  // Has its own context
    ProcessJobAsync("job-2")   // Has its own context
);
```
